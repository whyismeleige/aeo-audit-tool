...
class Crawler:
    ...
    async def _load_robots(self):
        try:
            ...
            logger.warning(
                f"Could not load robots.txt for {self.domain}:{e}. Proceeding without it."
            )
        ...
        try:
            ...
            async with self._semaphore:
                async with httpx.AsyncClient(
                    ...
                    logger.debug(
                        f"Successfully fetched HTML Content from URL: {url} - Status Code: {status_code}"
                    )
    ...
    async def crawl(self) -> list[CrawlResult]:
        ...
        logger.info(
            f"Crawl complete for {self.seed_url} -- {len(self.results)} pages fetched"
        )
